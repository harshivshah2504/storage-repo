...
def create_app() -> Flask:
    ...
    def archive_cover(release_id: int):
        # Lazy cover generation is intentionally restricted to images. Videos
        # are deliberately excluded — they would have to be downloaded in full
        # and decoded, and a single phone-shot 4K clip can blow the 512 MB
        # Render free tier on its own. For images we (a) stream the source to
        # disk before opening it and (b) let PIL's JPEG draft hint subsample
        # at decode time, capping per-request peak memory at ~5 MB.
        ...
        if not legacy_image:
            abort(404)
        # Hard cap on source asset size — a 200 MB raw photo would still decode
        # into hundreds of MB of pixel buffer, draft() or no draft().
        ...
        try:
            ...
            try:
                client.download_asset(legacy_image["id"], asset_path)
            except Exception:
                abort(404)
            data = thumbnails.make_cover_jpeg(asset_path)
            if not data:
                abort(404)
            try:
                ...
                # Persisting the cover is best-effort; the response below still
                # serves the just-generated bytes so the user sees the thumbnail.
                pass
            return Response(
                data,
                mimetype="image/jpeg",
    ...
    def upload_files():
        ...
        # Browser-supplied pre-generated cover (image archives only). The blob
        # is already a small JPEG, so reading it fully is cheap. Falls through
        # to None for non-image uploads or when client-side generation failed.
        cover_blob_file = request.files.get("cover_blob")
        ...
        if len(uploaded_files) == 1:
            ...
            if direct_upload_ok:
                ...
                try:
                    ...
                    _maybe_attach_cover_blob(client, result, cover_blob_file)
        ...
        cover_blob_path = None
        if cover_blob_file is not None and not append_tag:
            try:
                cover_blob_path = staging_root / "_browser_cover.jpg"
                cover_blob_file.save(str(cover_blob_path))
            except Exception:
                cover_blob_path = None
        ...
        payload = {
            ...
            "browser_cover_path": str(cover_blob_path) if cover_blob_path else None,
            "upload_origin": "browser-transfer",
...
def _result_release_id(result) -> Optional[int]:
    """Pull a release id off either an ArchiveManifest or a plain dict."""
    if result is None:
        return None
    if hasattr(result, "release_id"):
        try:
            return int(getattr(result, "release_id"))
...
def _maybe_attach_cover_blob(client, result, cover_blob_file) -> None:
    """Upload a browser-supplied cover JPEG as the release's `_cover.jpg`.

    Best-effort: any failure is swallowed so we never break the upload itself.
    """
    if cover_blob_file is None:
        return
    release_id = _result_release_id(result)
    if not release_id:
        return
    try:
        from . import thumbnails

        data = cover_blob_file.read()
        if not data:
            return
        client.upload_asset_bytes(
            release_id=release_id,
...
def _maybe_attach_cover_from_path(client, result, cover_path) -> None:
    """Same as ``_maybe_attach_cover_blob`` but reads the JPEG from disk."""
    if not cover_path:
        return
    release_id = _result_release_id(result)
    if not release_id:
...
def _run_upload_task(task_id: str) -> None:
    ...
    browser_cover_path = payload.pop("browser_cover_path", None)
    ...
    try:
        ...
        _maybe_attach_cover_from_path(client, result, browser_cover_path)
        _update_task(task_id, status="completed", result=_serialize(result))
    except Exception as exc:
        _update_task(task_id, status="failed", error=str(exc))
    finally:
...
def _cover_source_max_bytes() -> int:
    # Image source assets larger than this skip lazy cover generation, since a
    # huge raw photo can still decode into a multi-hundred-MB pixel buffer.
    return env_int("GITHUB_DRIVE_COVER_SOURCE_MAX_BYTES", 50 * 1024 * 1024)
